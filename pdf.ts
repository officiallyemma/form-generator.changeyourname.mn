...
if (isMobile()) {
    (document.getElementById('dateOfBirth') as HTMLInputElement).value = '1999-01-01';
}
...
function submitWithCaptcha(e, url) {
    ...
    grecaptcha.ready(function () {
        grecaptcha.execute('6LcgsXkrAAAAAOZ8g7jVWWZssZ6PUcfdltyyiiwy', { action: 'submit' }).then(function (token) {
            let form = document.getElementById('nameChangeForm') as HTMLFormElement;
            form.action = url;
            copyData();
...
(document.querySelector('#newSex') as HTMLInputElement).addEventListener('change', (e) => {
    if ((e.target as HTMLInputElement).value === (document.querySelector('#sexOnBirthRecords') as HTMLInputElement).value) {
